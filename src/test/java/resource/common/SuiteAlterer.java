...
import resource.utility.ExcelReader;
...
public class SuiteAlterer implements IAlterSuiteListener {
    ...
    public void alter(List<XmlSuite> suites) {
        try {
            ...
            if (System.getProperty("threadCount") != null) {
                GlobalVariables.THREAD_COUNT = System.getProperty("threadCount");
            } else {
                configFileReader = new ConfigFileReader(GlobalVariables.TEST_CONFIGURATION);
                GlobalVariables.THREAD_COUNT = configFileReader.getDataFromConfigurationFile("ThreadCount");
            }
            //Override the thread count
            suite.setThreadCount(Integer.parseInt(GlobalVariables.THREAD_COUNT));
            System.out.println("threadCount is set to " + GlobalVariables.THREAD_COUNT);
            //Support running a list of test cases
    ...
    public static List<Class<?>> find(String scannedPackage) throws UnsupportedEncodingException {
        ...
        File scannedDir = new File(URLDecoder.decode(scannedUrl.getFile().toString(), StandardCharsets.UTF_8.toString()));
